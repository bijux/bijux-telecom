...
| --- | --- |
| signal identity, component role, carrier constants, and registry record types | [signal processing](../06-bijux-gnss-signal/) owns registry contents, code generation, sample behavior, replicas, and DSP |
| acquisition and tracking request, result, lifecycle, uncertainty, and diagnostic records | [receiver execution](../05-bijux-gnss-receiver/) owns stage scheduling, lock decisions, reacquisition, and channel policy |
| navigation solution, residual, refusal, and provenance records | [navigation science](../04-bijux-gnss-nav/) owns parsing, orbit models, corrections, estimators, and solution acceptance |
| artifact headers and versioned payload validation | [repository infrastructure](../03-bijux-gnss-infra/) owns file discovery, run layout, manifests, provenance hashing, and persistence |
| configuration structures and validation reports | [command workflows](../01-bijux-gnss/) own operator syntax, workflow defaults, and report presentation |
