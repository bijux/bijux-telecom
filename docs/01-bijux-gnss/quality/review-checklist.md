...
- `src/lib.rs` re-exports an internal helper because it is convenient for a
  caller rather than because the command crate owns the public promise.
...
- Inspect `crates/bijux-gnss/docs/TESTS.md` before accepting a changed test
  shape.
- Inspect `crates/bijux-gnss/docs/FACADE.md` before accepting a new public
  export.
