...
pub fn solve_public_ab43_ppp(config: PppConfig) -> Result<Vec<PublicPppEpochSolution>, String> {
    let case = ab43_public_spp_case();
    let initial_position = case.observations.approx_position_ecef_m.ok_or_else(|| {
        "AB43 public RINEX observation fixture is missing APPROX POSITION XYZ".to_string()
    })?;
    ...
    for epoch in &case.observations.epochs {
        let Some(solution) = filter.solve_epoch(epoch, &case.navigation.ephemerides, &products)
        else {
            continue;
        };
        let gps_time = epoch
            .gps_time()
            .ok_or_else(|| format!("AB43 public epoch {} is missing GPS time", epoch.epoch_idx))?;
        let enu_error_m =
            station_enu_error_m((solution.ecef_x_m, solution.ecef_y_m, solution.ecef_z_m), truth);
...
pub fn public_ppp_convergence_metrics(
    ...
    let first_time = solutions.first().map(|solution| solution.gps_time).ok_or_else(|| {
        "public PPP convergence metrics require at least one solution epoch".to_string()
    })?;
    ...
    for solution in solutions {
        let elapsed_s = (solution.gps_time.week - first_time.week) as f64 * 604_800.0
    ...
    let final_3d_error_m =
        solutions.last().map(|solution| solution.enu_error_m.three_dimensional_m).ok_or_else(
            || "public PPP convergence metrics require a final solution epoch".to_string(),
        )?;
    ...
    case.observations
        ...
        .map(|epoch| {
            ...
            Ok(PublicPppEpochDiagnostics {
                ...
                dual_frequency_satellite_count: dual_frequency_satellite_count(
                    epoch,
                    &gps_satellites,
                ),
