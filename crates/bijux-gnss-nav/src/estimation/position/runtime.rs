...
pub struct PositionRuntime {
    #[allow(dead_code)]
    config: PositionRuntimeConfig,
    pending_diagnostic_events: Vec<bijux_gnss_core::api::DiagnosticEvent>,
...
impl PositionRuntime {
    pub fn new(config: PositionRuntimeConfig) -> Self {
    ...
    pub fn drain_diagnostic_events(&mut self) -> Vec<bijux_gnss_core::api::DiagnosticEvent> {
        std::mem::take(&mut self.pending_diagnostic_events)
    }

    fn record_event(&mut self, event: bijux_gnss_core::api::DiagnosticEvent) {
        self.pending_diagnostic_events.push(event);
    }
    ...
    pub fn solve_epoch_with_navigation_data_and_broadcast_ionosphere(
        ...
        self.pending_diagnostic_events.clear();
        ...
        let mut nav_epoch = NavSolutionEpoch {
            ...
            wls_solver_rank: Some(solution.solver_rank),
            wls_condition_number: solution.solver_condition_number,
...
pub fn supports_positioning_signal(
...
pub fn supported_positioning_signal(
    constellation: Constellation,
...
fn uncertainty_class_from_solution(solution: &NavSolutionEpoch) -> NavUncertaintyClass {
    ...
    } else if !precision_reporting::low_uncertainty_allowed(solution) {
        if sigma <= 10.0 {
...
fn geometry_threshold_violations(
    solution: &NavSolutionEpoch,
    thresholds: &PositionRuntimeThresholds,
) -> Vec<String> {
    let mut reasons = Vec::new();
    ...
    let weight = position_measurement_weight(
        ...
        WeightingConfig {
            model: match config.weighting.mode {
                PositionWeightingModel::Elevation => PositionWeightingModel::Elevation,
                PositionWeightingModel::Cn0 => PositionWeightingModel::Cn0,
                PositionWeightingModel::ElevationCn0 => PositionWeightingModel::ElevationCn0,
            },
...
fn build_provenance(
    ...
    let weighting_mode = if !config.weighting.enabled {
        ...
        match config.weighting.mode {
            PositionWeightingModel::Elevation => "elevation_sigma_weighted",
            PositionWeightingModel::Cn0 => "cn0_sigma_weighted",
            PositionWeightingModel::ElevationCn0 => "elevation_cn0_sigma_weighted",
        }
    };
