...
pub struct NavigationFilterConfig {
    pub tropo_enabled: bool,
    pub tropo_ztd_m: f64,
    pub thresholds: NavigationFilterThresholds,
}
...
fn prime_state_from_wls(
    ...
    let observations = obs
        ...
        .filter(|observation| {
            crate::api::position_observation_has_valid_satellite_time(observation, obs.t_rx_s.0)
        })
        .collect::<Vec<_>>();
...
fn estimate_klobuchar_delay_m(
    ...
    let model = crate::api::KlobucharModel::new(*coefficients);
    crate::api::IonosphereModel::delay_m(
        &model,
        receiver,
        azimuth_deg,
        elevation_deg,
