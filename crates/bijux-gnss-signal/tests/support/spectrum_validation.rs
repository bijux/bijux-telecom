...
use bijux_gnss_signal::api::{
    estimate_complex_power_spectral_density, estimate_power_spectral_density,
    expected_component_power_spectral_density, expected_filtered_component_power_spectral_density,
    find_deep_spectrum_nulls, summarize_power_spectral_density, FrontEndFirFilter,
    PowerSpectralDensityPoint, PowerSpectralDensitySummary, SpectrumEstimatorConfig, SpectrumNull,
};
...
impl SpectrumTolerance {
    ...
    pub const fn filtered_low_pass() -> Self {
        Self {
            center_frequency_hz: 8_000.0,
            occupied_bandwidth_ratio: 0.18,
            integrated_power: 0.08,
            symmetry_error: 0.03,
            null_frequency_hz: 30_000.0,
            shape_rms_error_db: 2.1,
        }
    }

    pub const fn filtered_band_pass() -> Self {
        Self {
            center_frequency_hz: 20_000.0,
            occupied_bandwidth_ratio: 0.20,
            integrated_power: 0.08,
            symmetry_error: 0.04,
            null_frequency_hz: 60_000.0,
            shape_rms_error_db: 2.4,
        }
    }
}
...
pub fn compare_filtered_component_spectrum(
    samples: &[Complex<f32>],
    sample_rate_hz: f64,
    component: SignalComponentSpec,
...
pub fn assert_spectrum_matches(
    ...
    assert!(
        (comparison.measured_summary.symmetry_error - comparison.expected_summary.symmetry_error)
            .abs()
            <= tolerance.symmetry_error,
        "symmetry error mismatch: measured={:?} expected={:?}",
        comparison.measured_summary,
        comparison.expected_summary
    );
    assert!(
...
fn compare_expected_spectrum(
    measured_points: Vec<PowerSpectralDensityPoint>,
    expected_points: Vec<PowerSpectralDensityPoint>,
    occupied_power_fraction: f64,
    null_threshold_db: f64,
) -> SpectrumComparison {
