...
use bijux_gnss_signal::api::{
    generate_ca_code, generate_ca_code_chips, Prn, CA_CODE_PERIOD_CHIPS,
};
...
fn exact_period_generator_matches_single_period_helper() {
    for prn in 1u8..=32 {
        ...
        let generated =
            generate_ca_code_chips(Prn(prn), CA_CODE_PERIOD_CHIPS).expect("valid PRN");
        assert_eq!(
            generated, expected,
            "single-period generation mismatch for PRN {prn}"
        );
    }
}
