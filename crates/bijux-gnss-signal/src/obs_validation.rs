...
use bijux_gnss_core::api::{
    Constellation, ConventionsConfig, ObsEpoch, ObsSatellite, SatId, Seconds, SignalBand,
...
mod tests {
    ...
    fn validate_obs_epochs_rejects_non_finite_cn0() {
        let epoch = observation_epoch_with_models_and_cn0("unavailable", f64::NAN);
    ...
    fn validate_obs_epochs_rejects_negative_cn0() {
        let epoch = observation_epoch_with_models_and_cn0("unavailable", -1.0);
    ...
    fn validate_obs_epochs_rejects_excessive_cn0() {
        let epoch = observation_epoch_with_models_and_cn0("unavailable", 100.0);
    ...
    fn validate_obs_epochs_rejects_unknown_observation_lock_state() {
        let mut epoch = observation_epoch_with_models_and_cn0("unavailable", 45.0);
        epoch.sats[0].metadata.observation_lock_state = "handoff".to_string();
    ...
    fn dual_frequency_satellite(
        ...
        ObsSatellite {
            ...
            error_model: None,
            metadata: ObsMetadata {
                doppler_model: "tracked_carrier_if_offset_hz".to_string(),
                observation_lock_state: "locked".to_string(),
                signal: signal_registry(sat.constellation, band, code)
                    .expect("signal definition must exist")
                    .spec,
                ..ObsMetadata::default()
            },
        }
    ...
    #[test]
    fn reports_complete_gps_l1_l2_pair() {
        let sat = SatId { constellation: Constellation::Gps, prn: 8 };
        let report = check_dual_frequency_observations(&[dual_frequency_epoch(vec![
            dual_frequency_satellite(sat, SignalBand::L1, SignalCode::Ca, true, true),
            dual_frequency_satellite(sat, SignalBand::L2, SignalCode::Py, true, true),
        ])]);
    ...
    #[test]
    fn reports_missing_frequency_for_incomplete_pair() {
        let sat = SatId { constellation: Constellation::Gps, prn: 8 };
        let report = check_dual_frequency_observations(&[dual_frequency_epoch(vec![
            dual_frequency_satellite(sat, SignalBand::L1, SignalCode::Ca, true, true),
        ])]);
        ...
        assert_eq!(report.pairs[0].status, DualFrequencyPairStatus::Incomplete);
        assert_eq!(report.pairs[0].issue, Some(DualFrequencyPairIssue::MissingFrequency));
    }

    #[test]
    fn reports_invalid_lock_flags_for_incomplete_pair() {
        let sat = SatId { constellation: Constellation::Galileo, prn: 11 };
        let report = check_dual_frequency_observations(&[dual_frequency_epoch(vec![
            dual_frequency_satellite(sat, SignalBand::E1, SignalCode::E1B, true, true),
            dual_frequency_satellite(sat, SignalBand::E5, SignalCode::E5a, true, false),
        ])]);
    ...
    #[test]
    fn reports_complete_beidou_pair() {
        let sat = SatId { constellation: Constellation::Beidou, prn: 21 };
        let report = check_dual_frequency_observations(&[dual_frequency_epoch(vec![
            dual_frequency_satellite(sat, SignalBand::B1, SignalCode::B1I, true, true),
            dual_frequency_satellite(sat, SignalBand::B2, SignalCode::B2I, true, true),
        ])]);
    ...
    #[test]
    fn limits_supported_pairs_by_constellation() {
        assert_eq!(
    ...
    #[test]
    fn rejects_signal_definition_mismatch() {
        let sat = SatId { constellation: Constellation::Gps, prn: 9 };
        let mut l2 = dual_frequency_satellite(sat, SignalBand::L2, SignalCode::Py, true, true);
        l2.metadata.signal = signal_registry(Constellation::Gps, SignalBand::L1, SignalCode::Ca)
            .expect("signal definition must exist")
            .spec;
    ...
    #[test]
    fn counts_mixed_constellation_pairs_independently() {
        let gps_sat = SatId { constellation: Constellation::Gps, prn: 3 };
        let galileo_sat = SatId { constellation: Constellation::Galileo, prn: 14 };
        let report = check_dual_frequency_observations(&[dual_frequency_epoch(vec![
            dual_frequency_satellite(gps_sat, SignalBand::L1, SignalCode::Ca, true, true),
            dual_frequency_satellite(gps_sat, SignalBand::L2, SignalCode::Py, true, true),
            dual_frequency_satellite(galileo_sat, SignalBand::E1, SignalCode::E1B, true, true),
            dual_frequency_satellite(galileo_sat, SignalBand::E5, SignalCode::E5a, true, true),
        ])]);

        assert_eq!(report.observed_pairs, 3);
