...
use bijux_gnss_core::api::{
    default_signal_band_for_constellation, Constellation, SatId, SignalBand,
};
...
pub(crate) fn acquisition_constellation_matches_config(
    ...
    default_acquisition_satellites(constellation)
        .into_iter()
        .next()
        .is_some_and(|sat| {
...
fn signal_band_matches_config(
    ...
    default_local_code_model(sat, signal_band)
        .ok()
        .flatten()
        .is_some_and(|model| {
