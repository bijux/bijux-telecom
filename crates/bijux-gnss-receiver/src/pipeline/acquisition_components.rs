...
use bijux_gnss_signal::api::{
    galileo_e1c_secondary_code, galileo_e5a_q_secondary_code, galileo_e5b_q_secondary_code,
    gps_l5_q_neumann_hoffman_code, resolved_signal_registry_entry,
    sample_galileo_e1_pilot_cboc_code, sample_galileo_e5a_q_primary_code,
...
impl AcquisitionComponentPlan {
    pub fn sample_local_code_period(
        ...
        match &self.local_code {
            ...
            AcquisitionComponentLocalCode::GalileoE1PilotCboc { sat } => {
                sample_galileo_e1_pilot_cboc_code(sat.prn, sample_rate_hz, 0.0, sample_count)
            }
...
impl AcquisitionComponentLocalCode {
    fn matches_identity(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            (Self::GalileoE1PilotCboc { sat: left }, Self::GalileoE1PilotCboc { sat: right }) => {
                left == right
            }
        ...
        (bijux_gnss_core::api::Constellation::Galileo, SignalBand::E1, SignalCode::E1B) => {
            let pilot_component = AcquisitionComponentPlan {
                ...
                secondary_code_period_signs: Some(
                    galileo_e1c_secondary_code().into_iter().collect(),
                ),
                local_code: AcquisitionComponentLocalCode::GalileoE1PilotCboc { sat },
            };
...
mod tests {
    ...
    #[test]
    fn galileo_e1_strategies_cover_data_pilot_and_component_sum() {
        let sat = SatId { constellation: Constellation::Galileo, prn: 11 };

        let strategies =
            acquisition_strategies_for_signal(sat, SignalBand::E1, SignalCode::E1B, None, 20)
                .expect("Galileo E1 acquisition strategies");
        ...
        assert_eq!(strategies[0].components[0].data_symbol_code_periods(), Some(1));
        assert_eq!(
            strategies[1].components.iter().map(|component| component.role).collect::<Vec<_>>(),
            vec![SignalComponentRole::Pilot]
        );
        ...
        assert_eq!(
            strategies[1].components[0]
                .secondary_code_period_signs()
                .expect("Galileo E1 pilot secondary code")
                .len(),
            25
        );
        assert_eq!(
