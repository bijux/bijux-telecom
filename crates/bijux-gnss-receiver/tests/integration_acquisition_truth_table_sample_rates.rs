...
fn acquisition_truth_table_covers_reference_low_and_high_rate_profiles() {
    ...
    let low_rate_report =
        validate_truth_guided_acquisition_table(&low_rate.config, &low_rate.frame, &low_rate.truth, 1, 2);
    let high_rate_report =
        validate_truth_guided_acquisition_table(&high_rate.config, &high_rate.frame, &high_rate.truth, 1, 2);
    ...
    assert_ne!(low_rate_prn3.expected_code_phase_samples, high_rate_prn3.expected_code_phase_samples);
