...
fn temp_file_path(name: &str) -> PathBuf {
    ...
    std::env::temp_dir().join(format!("bijux_{}_{}_{}.iq8", name, std::process::id(), nanos))
}

#[test]
fn signed_8bit_iq_matches_float_fixture_acquisition() {
    ...
    fs::write(&path, quantize_frame_to_i8_bytes(&frame.iq)).expect("write iq8 fixture");
    ...
    assert!(
        iq8_result.peak_mean_ratio / float_result.peak_mean_ratio > 0.65,
        "peak mean ratio degraded too far: {} vs {}",
        iq8_result.peak_mean_ratio,
        float_result.peak_mean_ratio
    );
    assert!(
        iq8_result.peak_second_ratio > 1.1,
        "peak second ratio fell below a usable separation margin: {}",
        iq8_result.peak_second_ratio,
    );

    fs::remove_file(&path).expect("remove iq8 fixture");
}

fn quantize_frame_to_i8_bytes(samples: &[num_complex::Complex<f32>]) -> Vec<u8> {
    let mut encoded = Vec::with_capacity(samples.len() * 2);
    for sample in samples {
        encoded.push(quantize_component(sample.re) as u8);
        encoded.push(quantize_component(sample.im) as u8);
    }
    encoded
}
