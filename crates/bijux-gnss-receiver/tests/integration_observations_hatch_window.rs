...
fn aligned_tracking_epoch(
    ...
    TrackEpoch {
        ...
        tracking_provenance: "observation_hatch_window".to_string(),
...
#[test]
fn observations_cap_hatch_gain_at_window_length() {
    let config = tracking_config();
    let sat = SatId { constellation: Constellation::Gps, prn: 29 };
    ...
    let carrier_delta_samples = 0.125 * lambda_m / meters_per_sample;
    let code_noise_samples = [0.0, 0.00015, -0.00015, 0.00015];
    let code_phase_samples = code_noise_samples
        .into_iter()
        .enumerate()
        .map(|(offset, noise_samples)| offset as f64 * carrier_delta_samples + noise_samples)
        .collect::<Vec<_>>();
    ...
    let raw_values_m = code_phase_samples
        .iter()
        .map(|code_phase_samples| aligned_pseudorange_m(&config, 68, *code_phase_samples))
        .collect::<Vec<_>>();
    let mut expected_values_m = vec![raw_values_m[0]];
    for index in 1..raw_values_m.len() {
        let predicted_m = expected_values_m[index - 1] + 0.125 * lambda_m;
        let smoothed_m = predicted_m + (raw_values_m[index] - predicted_m) / 2.0;
        expected_values_m.push(smoothed_m);
    }

    for index in 0..sats.len() {
        assert!(
            (sats[index].pseudorange_m.0 - expected_values_m[index]).abs() <= 1.0e-9,
            "epoch {index}: {:?}",
            sats[index]
        );
        assert_eq!(sats[index].metadata.smoothing_window, 2);
        assert_eq!(sats[index].metadata.smoothing_age, (index + 1) as u32);
        assert!(!sats[index].lock_flags.cycle_slip, "{:?}", sats[index]);
    }
}
