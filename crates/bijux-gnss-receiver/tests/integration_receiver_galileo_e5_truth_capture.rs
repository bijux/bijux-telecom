...
struct GalileoE5TruthCaptureFixture {
...
fn galileo_e5_truth_capture_fixture(
    scenario_id: &str,
    signal_code: SignalCode,
    data_bit_flip: bool,
    duration_s: f64,
) -> GalileoE5TruthCaptureFixture {
    ...
    GalileoE5TruthCaptureFixture { config, scenario, frame: scaled_frame, truth: bundle.truth }
}
...
fn receiver_finds_sustained_galileo_e5a_lock_window() {
    ...
    let (start, len) = longest_tracking_lock_window(&tracking.epochs, SignalCode::E5a)
        .expect("tracking lock window");
...
fn receiver_finds_sustained_galileo_e5b_lock_window() {
    ...
    let (start, len) = longest_tracking_lock_window(&tracking.epochs, SignalCode::E5b)
        .expect("tracking lock window");
    let lock_window = &tracking.epochs[start..start + len];

    assert!(len >= 10, "{tracking:#?}");
    assert!(lock_window.iter().all(|epoch| epoch.signal_code == SignalCode::E5b), "{tracking:#?}");
}
