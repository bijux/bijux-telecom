...
const ANOMALY_PRN: u8 = 29;
const ANOMALY_STEP_M: f64 = 80.0;
const ANOMALY_ONSET_EPOCH_INDEX: u64 = 120;

pub struct SatelliteClockAnomalyRun {
...
pub fn anomaly_prn() -> u8 {
    ANOMALY_PRN
}

pub fn static_satellite_clock_anomaly_run() -> SatelliteClockAnomalyRun {
    let truth_origin_ecef_m = geodetic_to_ecef(TRUTH_LAT_DEG, TRUTH_LON_DEG, TRUTH_ALT_M);
    let profile = receiver_motion_profile(
        "satellite_clock_anomaly_static",
        truth_origin_ecef_m,
        (0.0, 0.0, 0.0),
        RECEIVE_TIME_S,
        EPOCH_SPACING_S,
    ...
    SatelliteClockAnomalyRun {
        observations,
        solutions,
        anomaly_prn: ANOMALY_PRN,
        anomaly_onset_epoch_index: ANOMALY_ONSET_EPOCH_INDEX,
    }
}

pub fn clock_anomaly_health_events(solution: &NavSolutionEpoch) -> Vec<&NavHealthEvent> {
    solution
        .health
        .iter()
        .filter(|event| matches!(event, NavHealthEvent::SatelliteClockAnomaly { .. }))
        .collect()
}
...
fn truth_seeded_tracking_results_with_clock_step(
    ...
    signals
        ...
        .map(|signal| {
            let ephemeris = ephemerides
                ...
                .expect("clock anomaly ephemeris for signal");
            let epochs = truth_epochs
                .iter()
                .map(|truth_epoch| {
                    let anomaly_step_m = if signal.sat.prn == ANOMALY_PRN
                        && truth_epoch.epoch_idx >= ANOMALY_ONSET_EPOCH_INDEX
                    {
                        ANOMALY_STEP_M
                    } else {
                        0.0
                    ...
                    TrackEpoch {
                        ...
                        channel_uid: format!("Gps-{:02}-clock-anomaly", signal.sat.prn),
                        tracking_provenance: "navigation_satellite_clock_anomaly_truth".to_string(),
                ...
                .collect::<Vec<_>>();
            let first_epoch = epochs.first().expect("clock anomaly track epoch");
