...
const COMMON_CODE_STEP_M: f64 = 80.0;
const COMMON_DOPPLER_STEP_HZ: f64 = 480.0;

pub struct CommonCodeDopplerAnomalyRun {
    pub observations: Vec<ObsEpoch>,
    pub solutions: Vec<NavSolutionEpoch>,
    pub anomaly_onset_epoch_index: u64,
}

pub fn static_common_code_doppler_anomaly_run() -> CommonCodeDopplerAnomalyRun {
    let truth_origin_ecef_m = geodetic_to_ecef(TRUTH_LAT_DEG, TRUTH_LON_DEG, TRUTH_ALT_M);
    let profile = receiver_motion_profile(
        "common_code_doppler_anomaly_static",
        truth_origin_ecef_m,
        (0.0, 0.0, 0.0),
        RECEIVE_TIME_S,
        EPOCH_SPACING_S,
    ...
    let tracking = truth_seeded_tracking_results_with_common_anomaly(
        &config,
        &ephemerides,
        &signals,
...
pub fn common_anomaly_health_events(solution: &NavSolutionEpoch) -> Vec<&NavHealthEvent> {
    solution
        .health
        .iter()
        .filter(|event| matches!(event, NavHealthEvent::CommonCodeDopplerAnomaly { .. }))
        .collect()
}
...
fn truth_seeded_tracking_results_with_common_anomaly(
    ...
    let initial_receive_time_s = truth_epochs
        .first()
        .expect("common anomaly tracking requires a truth epoch")
        .receive_time_s;
    ...
    signals
        ...
        .map(|signal| {
            let ephemeris = ephemerides
                ...
                .expect("common anomaly ephemeris for signal");
            let epochs = truth_epochs
                .iter()
                .map(|truth_epoch| {
                    let anomaly_active = truth_epoch.epoch_idx >= ANOMALY_ONSET_EPOCH_INDEX;
                    let pseudorange_step_m = if anomaly_active { COMMON_CODE_STEP_M } else { 0.0 };
                    let doppler_step_hz = if anomaly_active { COMMON_DOPPLER_STEP_HZ } else { 0.0 };
                    ...
                    TrackEpoch {
                        ...
                        channel_uid: format!("Gps-{:02}-common-anomaly", signal.sat.prn),
                        tracking_provenance: "common_code_doppler_anomaly_truth".to_string(),
                ...
                .collect::<Vec<_>>();
            let first_epoch = epochs.first().expect("common anomaly track epoch");
