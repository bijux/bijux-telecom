...
use bijux_gnss_receiver::api::{
    sim::{
        ...
        validate_tracking_accuracy_budget, validate_truth_guided_tracking_table,
...
#[test]
fn tracking_accuracy_budget_enforces_hard_truth_thresholds() {
    ...
    let truth_table = validate_truth_guided_tracking_table(
        &fixture.config,
        &fixture.frame,
        &fixture.truth,
    ...
    let report = validate_tracking_accuracy_budget(&truth_table, budgets.tracking);
    ...
    for satellite in &report.satellites {
        ...
        assert!(
            satellite.max_carrier_error_hz <= report.max_carrier_error_hz + f64::EPSILON,
            "{satellite:?}"
        );
        assert!(
            satellite.max_doppler_error_hz <= report.max_doppler_error_hz + f64::EPSILON,
            "{satellite:?}"
        );
        assert!(
            satellite.max_code_phase_error_samples
                <= report.max_code_phase_error_samples + f64::EPSILON,
            "{satellite:?}"
        );
        assert!(
            satellite.max_cn0_error_db_hz <= report.max_cn0_error_db_hz + f64::EPSILON,
            "{satellite:?}"
        );
    }
}
...
fn build_truth_table_fixture(scenario_file: &str) -> TruthTableFixture {
    ...
    let bundle = build_iq16_capture_bundle(
        ...
        Some("integration tracking accuracy budget".to_string()),
    );
